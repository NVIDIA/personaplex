...
class PersonaPlexServer:
    ...
    def _do_load(self):
        with self._load_lock:
            ...
            try:
                ...
                self.progress = "Loading Moshi LM (14GB)..."
    ...
    def web(self):
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            audio_data_event = asyncio.Event()
            opus_ready_event = asyncio.Event()
            ...
            try:
                async with self.state.lock:
                    ...
                    logger.info("Handshake sent. Real-time phase.")
                                ...
                                if msg["type"] == "websocket.disconnect": break
                                    ...
                                    if len(data) > 0:
                                        if data[0] == 1: # Audio
                                            ...
                                            audio_data_event.set()
                                        elif data[0] == 6: # Ping
                                            await websocket.send_bytes(b"\x06")
                        except Exception as e:
                            logger.error(f"Recv Error: {e}")
                        finally: close = True; audio_data_event.set()

                    async def send_loop():
                        try:
                            while not close:
                                await opus_ready_event.wait()
                                opus_ready_event.clear()
                                msg = opus_writer.read_bytes()
                                while len(msg) > 0:
                                    await websocket.send_bytes(b"\x01" + msg)
                                    msg = opus_writer.read_bytes()
                                await asyncio.sleep(0.005) # Tiny sleep to avoid busy loop
                        except Exception as e:
                    ...
                    async def opus_loop():
                        ...
                        try:
                            all_pcm_data = None
                            while not close:
                                await audio_data_event.wait()
                                audio_data_event.clear()
                        ...
                        finally: close = True; opus_ready_event.set()
                ...
                logger.info("Cleanup closed connection")
