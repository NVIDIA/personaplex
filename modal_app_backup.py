...
image = (
    ...
    # Copy the local moshi directory into the container
    .add_local_dir(Path(__file__).parent / "moshi", remote_path="/root/moshi", copy=True)
    # Install the local moshi package so all imports work exactly as intended
...
class PersonaPlexServer:
    ...
    def web(self):
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            # Using asyncio.Queue for better decoupling and stability
            ...
            try:
                async with self.state.lock:
                    ...
                    logger.info("Stepping system prompts...")
                                ...
                                if msg["type"] == "websocket.disconnect": break
                                    ...
                                    if len(data) > 0:
                                        if data[0] == 1: # Audio
                                            ...
                                            await websocket.send_bytes(b"\x06")
                        except Exception as e:
                            logger.warning(f"Recv Error: {e}")
                        finally: close = True; await audio_queue.put(None)

                    async def send_loop():
                        try:
                            while not close:
                                # Wait for signal that new audio is ready in the opus_writer
                                item = await opus_queue.get()
                    ...
                    async def opus_loop():
                        ...
                        try:
                            all_pcm_data = None
                            while not close:
                                signal = await audio_queue.get()
                                if signal is None: break
                                
                                pcm = opus_reader.read_pcm()
                                while pcm is not None and pcm.shape[-1] > 0:
                        ...
                        finally: close = True; await opus_queue.put(None)

                    await asyncio.gather(recv_loop(), send_loop(), opus_loop())
...
def download_models():
    ...
    for f in [loaders.TEXT_TOKENIZER_NAME, loaders.MIMI_NAME, loaders.MOSHI_NAME]:
        hf_hub_download(repo, f, token=hf_token, local_dir=weights_dir, local_dir_use_symlinks=False)
    volume.commit()
